(function () {
  ...
  function puedeAdministrarPuntoOperacion(){return hasPermission('CONFIGURACION','GESTIONAR_PUNTO_BASE')&&['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||''));}
  function puedeCierreExcepcional(){return hasPermission('OPERACIONES','CIERRE_EXCEPCIONAL')&&['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||''));}
  function puedeReenviarAlertaAsignacion(){return ['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('NOTIFICACIONES','ENVIAR');}
  ...
  function puedeRevisarDocumentos(){return ['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').trim().toUpperCase());}
  ...
  function avisosEmergentesActivos(){
    if(!['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase()))return true;
  ...
  function puedeVerLineaTiempoRuta(){return ['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('RUTAS','LEER');}
  ...
  function puedeVerTrazabilidadRutas(){
    return ['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('RUTAS','LEER');
  }
  ...
  function canSelectGpsVehicles() {
    return ['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA','ROL-SUPERVISOR'].includes(currentUser?.ROL_ID);
  }
  ...
  async function probarConfiguracionConexionesWeb(form,button){
    const data=datosFormularioConexiones(form);
    const resultado=await api.request('testConnectionConfig',{data,cache:false});pintarResultadoPruebaConexiones(resultado);toast(resultado.todoCorrecto?'Conexiones disponibles':'Revise las conexiones',resultado.todoCorrecto?'Las tres comprobaciones terminaron correctamente.':'Al menos una dirección no respondió correctamente.',resultado.todoCorrecto?'success':'error');return resultado;
  }
  async function guardarConfiguracionConexionesWeb(form,button){
    const data=datosFormularioConexiones(form);
    const resultado=await api.request('saveConnectionConfig',{data,cache:false});if(resultado?.configuracionCliente)api.aplicarConfiguracionConexionCliente?.(resultado.configuracionCliente,{forzar:true});pintarResultadoPruebaConexiones(resultado?.pruebas||{});['settings'].forEach(section=>cacheVistasModulo.delete(section));toast('Configuración de conexión guardada',`Perfil v${resultado?.row?.VERSION_CONFIG||''} confirmado. Los nuevos inicios de sesión usarán esta configuración.`);setTimeout(()=>actualizarSeccionEnSegundoPlano('settings'),250);return resultado;
  }
  ...
  async function renderSettings(){
    ...
    const esAdminConexion=['ROL-SYSADMIN','ROL-ADMIN','ROL-GERENCIA'].includes(String(currentUser?.ROL_ID||'').toUpperCase());let perfilConexion=null;
    ...
    const puedeEditarConexion=Boolean(perfil.PUEDE_EDITAR_SENSIBLE),soloLectura=puedeEditarConexion?'':'readonly',botonesBloqueados=!puedeEditarConexion||perfil.REQUIERE_SQL;
    const connectionManager=esAdminConexion?`<section class="connection-manager-shell"><article class="card connection-manager-card"><div class="card-header"><div><span class="eyebrow">ADMINISTRACIÓN / GERENCIA · CONTROL SENSIBLE</span><h3>Configuración de Conexiones</h3><p>Las direcciones pueden consultarse, pero solo se modifican con autorización sensible otorgada por el Administrador del Sistema.</p></div><span class="status ${perfil.ERROR||perfil.REQUIERE_SQL||!puedeEditarConexion?'warning':'ok'}">${perfil.ERROR?'Revisar':perfil.REQUIERE_SQL?'Instalar SQL':puedeEditarConexion?`Autorizado · v${esc(perfil.VERSION_CONFIG||cfgLocal.version||1)}`:'Bloqueado'}</span></div>${perfil.ERROR?`<div class="connection-config-alert error">${esc(perfil.ERROR)}</div>`:''}${perfil.REQUIERE_SQL?'<div class="connection-config-alert warning">Ejecute SQL_SYSADMIN_CLAVES_EXTERNAS_4.3.20.sql y el SQL del módulo de conexiones si aún no está instalado.</div>':''}${!puedeEditarConexion?'<div class="connection-config-alert warning"><b>Edición bloqueada.</b> La casilla “Modificar URL de conexiones” debe ser habilitada por el Administrador del Sistema.</div>':''}<form id="connectionConfigForm" class="form-grid connection-config-form"><label class="field full"><span>Directorio de acceso / inicio de sesión</span><input name="DIRECTORIO_URL" type="url" required autocomplete="off" value="${esc(perfil.DIRECTORIO_URL||cfgLocal.directorioUrl||'')}" placeholder="https://.../exec" ${soloLectura}><small>Resuelve el RUT y obtiene internamente la conexión principal del sistema.</small></label><label class="field full"><span>Servicio de actualizaciones</span><input name="ACTUALIZACIONES_URL" type="url" required autocomplete="off" value="${esc(perfil.ACTUALIZACIONES_URL||'')}" placeholder="https://.../exec" ${soloLectura}><small>Dirección utilizada por el servidor para publicar y validar nuevas versiones móviles.</small></label><label class="field full"><span>Conexión principal de respaldo (opcional)</span><input name="API_RESPALDO_URL" type="url" autocomplete="off" value="${esc(perfil.API_RESPALDO_URL||cfgLocal.apiRespaldoUrl||'')}" placeholder="https://..." ${soloLectura}><small>Se conserva en los dispositivos como referencia de recuperación. No se usa automáticamente para escrituras.</small></label><input type="hidden" name="ESTADO" value="ACTIVA"><div class="connection-config-meta full"><span>Última actualización: <b>${perfil.ACTUALIZADO_EN?fmtDate(perfil.ACTUALIZADO_EN,true):'Sin cambios registrados'}</b></span><span>Directorio local de este navegador: <b>${cfgLocal.usaPredeterminado?'Respaldo incluido':'Sincronizado'}</b></span></div><div class="connection-config-test full" data-connection-config-status><div class="connection-test-placeholder">${puedeEditarConexion?'Autorización SYSADMIN vigente. Pruebe antes de guardar.':'Modo solo lectura.'}</div></div><div class="form-actions full"><button class="btn soft" type="button" data-test-connection-config ${botonesBloqueados?'disabled':''}>Probar conexiones</button><button class="btn primary" type="submit" ${botonesBloqueados?'disabled':''}>Guardar y activar</button></div></form><p class="helper">La API exige la autorización sensible otorgada por SYSADMIN. Tener rol Administrador o Gerencia por sí solo no autoriza cambios.</p></article></section>`:'';
    ...
    connectionManager+
    `${['ROL-SYSADMIN','ROL-ADMIN'].includes(String(currentUser?.ROL_ID||'').toUpperCase())?`<section class="connection-admin-shell"><article class="card"><div class="card-header"><div><span class="eyebrow">EMPRESA ACTUAL</span><h3>Conexión empresarial del dispositivo</h3><p>La empresa se resuelve por RUT desde el directorio configurado. La conexión real de la base permanece oculta para usuarios finales.</p></div><span class="status ok">✓ Conexión establecida</span></div><div class="info-grid"><div class="info-item"><span>Empresa</span><b>${esc(empresaConexion.nombre||'Configurada')}</b></div><div class="info-item"><span>RUT</span><b>${esc(empresaConexion.rut||'Registrado internamente')}</b></div></div><div class="form-actions"><button class="btn primary" type="button" data-open-company-connection>Cambiar empresa por RUT</button></div><p class="helper">Al cambiar de empresa se cerrará la sesión actual y se solicitará el acceso correspondiente a la nueva base.</p></article></section>`:''}`+
    ...
    `${['ROL-SYSADMIN','ROL-ADMIN'].includes(String(currentUser?.ROL_ID||'').toUpperCase())&&hasPermission('CONFIGURACION','RESPALDO_GENERAL')?'<section class="backup-database-shell"><article class="card backup-database-card"><div class="card-header"><div><span class="eyebrow">RESPALDO GENERAL</span><h3>Descargar Base de Datos en XLSX</h3><p>Genera un libro Excel con una hoja por cada tabla y todos los registros disponibles hasta la fecha.</p></div><span class="status ok">Solo Administrador</span></div><div class="backup-feature-list"><span>✓ Incluye registros activos e históricos</span><span>✓ Protege contraseñas, tokens y credenciales</span><span>✓ Agrega resumen de tablas y totales</span></div><div class="backup-status" data-backup-status>Listo para generar el respaldo.</div><div class="form-actions"><button class="btn primary" type="button" data-backup-database>⬇ Descargar respaldo XLSX</button></div></article></section>':''}`+
  ...
  function rolAdmiteConexionSensible(user){
    const rol=String(user?.ROL_ID||user?.ROL_NOMBRE||'').toUpperCase();
    return ['ROL-ADMIN','ADMINISTRADOR','ROL-GERENCIA','GERENCIA'].includes(rol);
  }
  function autorizacionConexionSensible(user){return user?.AUTORIZA_CONEXIONES_SENSIBLES===true||String(user?.AUTORIZA_CONEXIONES_SENSIBLES||'').toUpperCase()==='SI';}
  function permissionMatrixMarkup(user){
    const admin=['ROL-SYSADMIN','ROL-ADMIN'].includes(String(user.ROL_ID||'').toUpperCase())||String(user.ROL_NOMBRE||'').toUpperCase()==='ADMINISTRADOR';
  ...
  async function openUserPermissionsModal(userId){
    ...
    form.onsubmit=async event=>{
      ...
      const sensibleInput=form.elements.AUTORIZA_CONEXIONES_SENSIBLES,sensibleOriginal=autorizacionConexionSensible(user),puedeCambiarSensible=Boolean(sensibleInput&&!sensibleInput.disabled),sensible=puedeCambiarSensible?Boolean(sensibleInput.checked):sensibleOriginal;
      await conCargaBoton(button,'Guardando y verificando…',async()=>{
        try{
          const dataPermisos={USUARIO_ID:userId,MODO_PERMISOS:mode,PERMISOS:permissions,INICIALIZAR_DESDE_ROL:'NO'};
          if(puedeCambiarSensible)dataPermisos.AUTORIZA_CONEXIONES_SENSIBLES=sensible?'SI':'NO';
          const result=await api.request('saveUserPermissions',{data:dataPermisos});
          ...
          const sensibleConfirmado=autorizacionConexionSensible(confirmed);
          if(!exact||(puedeCambiarSensible&&sensibleConfirmado!==sensible))throw new Error('PERMISOS_USUARIO_NO_PERSISTIERON');
