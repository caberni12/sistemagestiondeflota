(function(){
  ...
  function permitido(modulo){
    ...
    const administrador=rol==='ROL-SYSADMIN'||rol==='SYSADMIN'||rol==='ADMINISTRADOR DEL SISTEMA'||rol==='ROL-ADMIN'||rol==='ADMINISTRADOR';
  ...
  function esAdministradorMenu(){const rol=String(usuario?.ROL_ID||usuario?.ROL_NOMBRE||'').trim().toUpperCase();return rol==='ROL-SYSADMIN'||rol==='SYSADMIN'||rol==='ADMINISTRADOR DEL SISTEMA'||rol==='ROL-ADMIN'||rol==='ADMINISTRADOR'||rol==='ROL-GERENCIA'||rol==='GERENCIA';}
  ...
  function puedeAvisos(modulo){if(!usuario)return false;const permisos=Array.isArray(usuario.PERMISOS)?usuario.PERMISOS:[];const rol=String(usuario.ROL_ID||usuario.ROL_NOMBRE||'').trim().toUpperCase();return rol==='ROL-SYSADMIN'||rol==='SYSADMIN'||rol==='ROL-ADMIN'||rol==='ADMINISTRADOR'||permisos.includes('*:*')||permisos.includes(`${modulo}:LEER`);}
