(function(){
  ...
  const sysadminForm=$('#formularioAccesoSysadmin');
  const sysadminButton=$('#botonAccesoSysadmin');
  const mensajeSysadmin=$('#mensajeSysadmin');
  ...
  const errores={
    ...
    ,CLAVE_SYSADMIN_REQUERIDA:'Ingrese la clave de Administrador del Sistema.'
    ,CLAVE_SYSADMIN_INVALIDA:'La clave de Administrador del Sistema no es válida.'
    ,CLAVE_SYSADMIN_BLOQUEADA:'La clave de Administrador del Sistema está bloqueada.'
    ,CLAVE_SYSADMIN_REVOCADA:'La clave de Administrador del Sistema fue revocada y no puede volver a utilizarse.'
    ,CLAVE_SYSADMIN_EXPIRADA:'La clave de Administrador del Sistema expiró.'
    ,RESPUESTA_SYSADMIN_INVALIDA:'El acceso SYSADMIN no pudo validarse correctamente.'
  ...
  function entrar(){location.replace('main.html?v=4.3.22');}
  function mostrarPreconfiguracion(){companyForm.classList.add('oculto');loginForm.classList.add('oculto');sysadminForm?.classList.add('oculto');setupForm.classList.remove('oculto');cambiarEstado('Preconfiguración requerida','preconfig');$('#detalleServicio').textContent='Sin usuarios registrados';setTimeout(()=>setupForm.elements.nombreEmpresa?.focus(),80);}
  function mostrarAcceso(){companyForm.classList.add('oculto');setupForm.classList.add('oculto');sysadminForm?.classList.add('oculto');loginForm.classList.remove('oculto');}
  function mostrarSeleccionEmpresa(){loginForm.classList.add('oculto');setupForm.classList.add('oculto');sysadminForm?.classList.add('oculto');companyForm.classList.remove('oculto');setTimeout(()=>$('#rutConexionEmpresa')?.focus(),80);}
  ...
  $('#abrirAccesoSysadmin')?.addEventListener('click',()=>{companyForm.classList.add('oculto');loginForm.classList.add('oculto');setupForm.classList.add('oculto');sysadminForm.classList.remove('oculto');sysadminForm.reset();ocultarMensaje(mensajeSysadmin);setTimeout(()=>$('#rutSysadmin')?.focus(),80);});
  $('#volverAccesoEmpresa')?.addEventListener('click',()=>{sysadminForm.reset();ocultarMensaje(mensajeSysadmin);mostrarSeleccionEmpresa();});
  $('#mostrarClaveSysadmin')?.addEventListener('click',()=>{const input=$('#claveSysadmin');input.type=input.type==='password'?'text':'password';});
  sysadminForm?.addEventListener('submit',async event=>{
    event.preventDefault();ocultarMensaje(mensajeSysadmin);if(!sysadminForm.reportValidity())return;bloquear(sysadminButton,true,'Validando…','Acceder como Administrador del Sistema');
    const datos=Object.fromEntries(new FormData(sysadminForm).entries());
    try{
      const ip=await (api.getClientIp?.().catch(()=> '')||Promise.resolve(''));
      const resultado=await api.iniciarSesionSysadminExterno(datos.rut,datos.clave,{ipPublica:ip});
      // La clave se descarta inmediatamente del DOM después de validarse.
      sysadminForm.elements.clave.value='';
      cambiarEstado('Acceso SYSADMIN autorizado','conectado');mostrarMensaje(`Acceso superior autorizado para ${resultado.empresa?.nombre||'la empresa'}.`,'exito',mensajeSysadmin);setTimeout(()=>entrar(),350);
    }catch(error){sysadminForm.elements.clave.value='';mostrarMensaje(textoError(error),'error',mensajeSysadmin);cambiarEstado('Acceso SYSADMIN rechazado','error');}
    finally{bloquear(sysadminButton,false,'Validando…','Acceder como Administrador del Sistema');}
  });
  ...
  setupForm.addEventListener('submit',async event=>{
    ...
    try{
      ...
      const ipPromise=api.getClientIp?.().catch(()=> '')||Promise.resolve('');const resultado=await api.request('login',{correo:datos.correo,contrasena:datos.contrasena,RUT_EMPRESA:datos.rutEmpresa||''});
  ...
  // La recuperación manual del directorio fue retirada: la URL directora es un parámetro sensible administrado por SYSADMIN.
