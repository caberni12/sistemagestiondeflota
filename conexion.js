(function () {
  ...
  const accionesAplicacion = Object.freeze({
    health:'salud', status:'estadoSistema', bootstrap:'instalacionInicial', login:'iniciarSesion', sysadminLogin:'iniciarSesionSysadmin',
  ...
  const DIRECTORIOS_LEGADOS_SGF = new Set([
    'https://script.google.com/macros/s/AKfycbztB10E6yNld603mdtuqJ_53q1PAJHoXm3en7Q-6UmP0JTRGN-0yMENoPcldMYIMgAjZQ/exec',
    'https://script.google.com/macros/s/AKfycbwQ84N6sYM5QLL05N_fB5YRvtayf5EIzABP9nZbrLoE5aKTEBPcaxHG1XXRuoe-1ss60w/exec',
    'https://script.google.com/macros/s/AKfycbxIUJ3Gsq4mTrnXfDstuSC-gw3uCL_r6M15vWN8_qHqBo2HtfKBOia3GhyKE40DPmzAAw/exec'
  ]);
  function migrarDirectorioLegado(url) {
    const valor = String(url || '').trim();
    return DIRECTORIOS_LEGADOS_SGF.has(valor)
      ? String(config.DIRECTORIO_EMPRESAS_URL || '').trim()
      : valor;
  }
  ...
  function cargarConfiguracionConexionesLocal() {
    try {
      ...
      const directorioOriginal = urlHttpsValida(guardada.directorioUrl) ? String(guardada.directorioUrl).trim() : '';
      const directorioUrl = migrarDirectorioLegado(directorioOriginal);
      if (directorioOriginal && directorioUrl !== directorioOriginal) {
        guardada.directorioUrl = directorioUrl;
        guardada.guardadaEn = new Date().toISOString();
        try { localStorage.setItem(claveConfiguracionConexionesLocal, JSON.stringify(guardada)); } catch (_) {}
      }
  ...
  async function solicitarDirectorioPost(accion, datos={}) {
    if (!directorioEmpresasConfigurado()) throw new Error('DIRECTORIO_EMPRESAS_NO_CONFIGURADO');
    const controller=new AbortController(),temporizador=setTimeout(()=>controller.abort(),Number(config.TIEMPO_ESPERA_DIRECTORIO_MILISEGUNDOS||18000));
    try{
      const response=await fetch(direccionDirectorioActual(),{method:'POST',redirect:'follow',credentials:'omit',cache:'no-store',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({accion,...datos}),signal:controller.signal});
      const text=await response.text();let result;try{result=JSON.parse(text);}catch(_){throw new Error('RESPUESTA_DIRECTORIO_INVALIDA');}
      if(!result||result.ok!==true)throw new Error(String(result?.error||'ERROR_DIRECTORIO'));
      return result;
    }catch(error){if(error?.name==='AbortError')throw new Error('TIEMPO_DE_ESPERA_DIRECTORIO');throw error;}finally{clearTimeout(temporizador);}
  }

  async function iniciarSesionSysadminExterno(rutIngresado,claveAcceso,{ipPublica=''}={}){
    const rut=normalizarRutEmpresa(rutIngresado),clave=String(claveAcceso||'').trim();
    if(!rutEmpresaValido(rut))throw new Error('RUT_EMPRESA_INVALIDO');
    if(!clave)throw new Error('CLAVE_SYSADMIN_REQUERIDA');

    // 1) El Directorio externo sigue siendo la fuente de verdad para RUT,
    // estado de empresa y URL_CONEXION. No se usa una flotas-api fija.
    const directorio=await solicitarDirectorioJsonp(rut);
    if(!directorio||directorio.ok!==true)throw new Error(String(directorio?.error||'EMPRESA_NO_REGISTRADA'));
    const estadoEmpresa=String(directorio.empresa?.estado||'').trim().toUpperCase();
    const nombre=String(directorio.empresa?.nombre||'').trim();
    const rutFormateado=String(directorio.empresa?.rut||rut).trim();
    const url=String(directorio.conexion?.url||'').trim();
    if(estadoEmpresa!=='ACTIVA')throw new Error('EMPRESA_BLOQUEADA');
    if(!nombre||!urlHttpsValida(url))throw new Error('RESPUESTA_DIRECTORIO_INVALIDA');

    // 2) La clave SYSADMIN se valida en la flotas-api de ESA empresa.
    const controller=new AbortController(),temporizador=setTimeout(()=>controller.abort(),Number(config.TIEMPO_ESPERA_MILISEGUNDOS||20000));
    let respuestaApi;
    try{
      const response=await fetch(url,{method:'POST',redirect:'follow',credentials:'omit',cache:'no-store',headers:{'Content-Type':'application/json;charset=utf-8','Accept':'application/json'},body:JSON.stringify({accion:'iniciarSesionSysadmin',action:'iniciarSesionSysadmin',RUT_EMPRESA:rutFormateado,CLAVE_ACCESO:clave,IP_PUBLICA:ipPublica,AGENTE_NAVEGADOR:navigator.userAgent}),signal:controller.signal});
      const text=await response.text();try{respuestaApi=JSON.parse(text);}catch(_){throw new Error('RESPUESTA_SYSADMIN_INVALIDA');}
      if(!response.ok||!respuestaApi||respuestaApi.ok!==true)throw new Error(String(respuestaApi?.error||'CLAVE_SYSADMIN_INVALIDA'));
    }catch(error){if(error?.name==='AbortError')throw new Error('TIEMPO_DE_ESPERA_DIRECTORIO');throw error;}finally{clearTimeout(temporizador);}

    const sesion=respuestaApi.data||{};
    if(!sesion.token||String(sesion.user?.ROL_ID||'').toUpperCase()!=='ROL-SYSADMIN')throw new Error('RESPUESTA_SYSADMIN_INVALIDA');
    conexionEmpresa={nombre,rut:rutFormateado,url,guardadaEn:new Date().toISOString(),accesoSysadmin:true,estadoDirectorio:estadoEmpresa};
    localStorage.setItem(config.CLAVE_CONEXION_EMPRESA,JSON.stringify(conexionEmpresa));
    setAuth({token:sesion.token,sessionId:sesion.sessionId||'',user:sesion.user,expiresAt:sesion.expiresAt||'',sysadmin:true});
    limpiarCache();window.dispatchEvent(new CustomEvent('flotas:conexion-empresa-cambiada',{detail:obtenerConexionEmpresa()}));
    return {empresa:obtenerConexionEmpresa(),sesion};
  }
